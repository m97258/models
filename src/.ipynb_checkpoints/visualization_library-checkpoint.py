...
probability_column                  = constants.NAME_PROBABILITY_COLUMN
...
def plot_errors(Ytest, Y_probabilities, title, errors_titles, labels_titles):
    """
   This function plots the errors type I and type II of a classification model at different score levels.
    
    Inputs:
    - errors_table: a dataframe with the errors type I and type II of a series of observations
    - title: the title of the graph
    - errors_titles: a list with the titles of the two types of errors
    - labels_titles: a list with the titles of the axis of the graph
    
    Outputs:
    - a graph displayed on the screen 
    """  
    
    Ytrue_versus_Ypred = pd.DataFrame()
    Ytrue_versus_Ypred[target_column]      = Ytest
    Ytrue_versus_Ypred[probability_column] = Y_probabilities
    Ranking_probabilities_adding     = Ytrue_versus_Ypred[Ytrue_versus_Ypred[target_column]==1][probability_column].sort_values()
    Ranking_probabilities_non_adding = Ytrue_versus_Ypred[Ytrue_versus_Ypred[target_column]==0][probability_column].sort_values()
    errors_table = modeling_library.build_errors_table(Ranking_probabilities_adding, Ranking_probabilities_non_adding, 10000)
    f = plt.figure(figsize=(16, 8))
    plt.plot(errors_table[name_accumulated_probability_column], errors_table[name_error_accumulated_I_column], color='r', \
             label=errors_titles[0])
    plt.plot(errors_table[name_accumulated_probability_column], errors_table[name_error_accumulated_II_column], color='g', \
             label=errors_titles[1])
    plt.legend()
    plt.legend(fontsize=16)
    f.suptitle(title, fontsize=18)
    plt.xlabel(labels_titles[0], fontsize=16)
    plt.ylabel(labels_titles[1], fontsize=16)
    plt.show()
